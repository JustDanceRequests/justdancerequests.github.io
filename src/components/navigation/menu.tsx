  ...
  return (
    <nav class='bg-gray-100 border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900'>
      <div class='container flex flex-wrap justify-between items-center mx-auto'>
        <div class='flex-1 hidden w-full md:flex md:w-auto justify-center'>
          <ul class='flex flex-col items-center md:flex-row md:space-x-8 md:text-sm md:font-medium dark:bg-gray-800 md:dark:bg-gray-900'>
            {props.entries.map((entry) => (
              <MenuEntry name={entry.name} href={entry.href} />
            ))}
          </ul>
        </div>

        <a href='/' class='flex-1 flex items-center ml-6 md:ml-0 md:justify-center'>
          <img src='/images/logo.png' class='mr-3 h-9 sm:h-12' alt='JDR Logo' />
        </a>

        <div class='flex-1 flex flex-row items-center justify-center space-x-12'>
